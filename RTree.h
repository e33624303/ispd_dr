...
public:
  ...
  int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], CallbackType a_resultCallback, ContextType a_context);
  ...
  public:
    ...
    DATATYPE& operator*()
    {
      ASSERT(IsNotNull() && __FILE__ && ": " && __LINE__ && '\n');
  ...
  private:
    ...
    void Push(Node* a_node, int a_branchIndex)
    {
      ...
      ++m_tos;
      ASSERT((m_tos <= MAX_STACK) && __FILE__ && ": " && __LINE__ && '\n');
    }
    
    /// Pop element off iteration stack (For internal use only)
    StackElement& Pop()
    {
      ASSERT((m_tos > 0) && __FILE__ && ": " && __LINE__ && '\n');
      --m_tos;
...
protected:
  ...
  template <typename CallbackType, typename ContextType>
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, CallbackType a_resultCallback, ContextType a_context);
...
public:
  ...
  {
    ASSERT((m_file) && __FILE__ && ": " && __LINE__ && '\n');
...
RTREE_QUAL::RTree()
{
  ASSERT((MAXNODES > MINNODES) && __FILE__ && ": " && __LINE__ && '\n');
  ASSERT((MINNODES > 0) && __FILE__ && ": " && __LINE__ && '\n');


  // We only support machine word size simple data type eg. integer index or object pointer.
  // Since we are storing as union with non data branch
  ASSERT((sizeof(DATATYPE) == sizeof(void*) || sizeof(DATATYPE) == sizeof(int)) && __FILE__ && ": " && __LINE__ && '\n');
...
void RTREE_QUAL::Insert(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], const DATATYPE& a_dataId)
{
  ...
  for(int index=0; index<NUMDIMS; ++index)
  {
    ASSERT((a_min[index] <= a_max[index]) && __FILE__ && ": " && __LINE__ && '\n');
  }
#endif //_DEBUG
...
RTREE_TEMPLATE
template <typename CallbackType, typename ContextType>
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], CallbackType a_resultCallback, ContextType a_context)
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
...
{
  ASSERT((a_node) && __FILE__ && ": " && __LINE__ && '\n');
  ASSERT((a_node->m_level >= 0) && __FILE__ && ": " && __LINE__ && '\n');
...
{
  ASSERT((a_rect && a_node && a_newNode) && __FILE__ && ": " && __LINE__ && '\n');
  ASSERT((a_level >= 0 && a_level <= a_node->m_level) && __FILE__ && ": " && __LINE__ && '\n');
...
{
  ASSERT((a_rect && a_root) && __FILE__ && ": " && __LINE__ && '\n');
  ASSERT((a_level >= 0 && a_level <= (*a_root)->m_level) && __FILE__ && ": " && __LINE__ && '\n');
#ifdef _DEBUG
  for(int index=0; index < NUMDIMS; ++index)
  {
    ASSERT((a_rect->m_min[index] <= a_rect->m_max[index]) && __FILE__ && ": " && __LINE__ && '\n');
  }
#endif //_DEBUG  
...
{
  ASSERT((a_branch) && __FILE__ && ": " && __LINE__ && '\n');
  ASSERT((a_node) && __FILE__ && ": " && __LINE__ && '\n');
  ...
  {
    ASSERT(a_newNode && __FILE__ && ": " && __LINE__ && '\n');
...
{
  ASSERT((a_node && (a_index >= 0) && (a_index < MAXNODES)) && __FILE__ && ": " && __LINE__ && '\n');
  ASSERT((a_node->m_count > 0) && __FILE__ && ": " && __LINE__ && '\n');
...
{
  ASSERT((a_rect && a_node) && __FILE__ && ": " && __LINE__ && '\n');
...
{
  ASSERT((a_rectA && a_rectB) && __FILE__ && ": " && __LINE__ && '\n');
...
void RTREE_QUAL::SplitNode(Node* a_node, Branch* a_branch, Node** a_newNode)
{
  ...
  *a_newNode = AllocNode();
  ...
  ASSERT(((a_node->m_count + (*a_newNode)->m_count) == parVars->m_total) && __FILE__ && ": " && __LINE__ && '\n');
}
...
{
  ASSERT((a_rect) && __FILE__ && ": " && __LINE__ && '\n');
  ...
  ASSERT((volume >= (ELEMTYPEREAL)0) && __FILE__ && ": " && __LINE__ && '\n');
...
void RTREE_QUAL::GetBranches(Node* a_node, Branch* a_branch, PartitionVars* a_parVars)
{
  ...
  ASSERT((a_node->m_count == MAXNODES) && __FILE__ && ": " && __LINE__ && '\n');
...
{
  ASSERT((a_parVars) && __FILE__ && ": " && __LINE__ && '\n');
...
{
  ASSERT((a_parVars) && __FILE__ && ": " && __LINE__ && '\n');
  /*printf("a_parVars->m_taken[a_index] : %d\n", a_parVars->m_taken[a_index]);
  printf("MAXNODES : %d , a_index : %d , a_group : %d \n", MAXNODES, a_index);*/

  //if (a_parVars->m_taken[a_index] == 1 )return;
  ASSERT((!a_parVars->m_taken[a_index]) && __FILE__ && ": " && __LINE__ && '\n');
...
{
  ASSERT((a_rect && a_root) && __FILE__ && ": " && __LINE__ && '\n');
  ASSERT((*a_root) && __FILE__ && ": " && __LINE__ && '\n');
  ...
  if(!RemoveRectRec(a_rect, a_id, *a_root, &reInsertList))
  {
    ...
    if((*a_root)->m_count == 1 && (*a_root)->IsInternalNode())
    {
      ...
      ASSERT((tempNode) && __FILE__ && ": " && __LINE__ && '\n');
      FreeNode(*a_root);
      *a_root = tempNode;
    }
    return false;
...
{
  ASSERT((a_rect && a_node && a_listNode) && __FILE__ && ": " && __LINE__ && '\n');
  ASSERT((a_node->m_level >= 0) && __FILE__ && ": " && __LINE__ && '\n');
...
RTREE_TEMPLATE
template <typename CallbackType, typename ContextType>
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, CallbackType a_resultCallback, ContextType a_context)
{
  ASSERT((a_node) && __FILE__ && ": " && __LINE__ && '\n');
  ASSERT((a_node->m_level >= 0) && __FILE__ && ": " && __LINE__ && '\n');
  ASSERT((a_rect) && __FILE__ && ": " && __LINE__ && '\n');
