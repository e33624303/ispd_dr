...
namespace Lef{
	...
	/*
	Temp Structure
	Using these to assign Information to Design
	*/
	LayerInfo TempLayer;
	MACRO TempMACRO;
	Via TempVia;
	...
	void lefVia(lefiVia *via) {
		...
		Via ISPD_Via;
		ISPD_Via.Name = via->lefiVia::name();
		if (via->lefiVia::hasDefault())
			ISPD_Via.HasDefault = true;
		else ISPD_Via.HasDefault = false;
		if (via->lefiVia::numLayers() == 3) {
			ISPD_Via.BotLayerName = via->lefiVia::layerName(0);
			ISPD_Via.BotLayerRect.LB = make_pair(via->lefiVia::xl(0, 0), via->lefiVia::yl(0, 0));
			ISPD_Via.BotLayerRect.RT = make_pair(via->lefiVia::xh(0, 0), via->lefiVia::yh(0, 0));
			ISPD_Via.ViaLayerName = via->lefiVia::layerName(1);
			ISPD_Via.ViaLayerRect.LB = make_pair(via->lefiVia::xl(1, 0), via->lefiVia::yl(1, 0));
			ISPD_Via.ViaLayerRect.RT = make_pair(via->lefiVia::xh(1, 0), via->lefiVia::yh(1, 0));
			ISPD_Via.TopLayerName = via->lefiVia::layerName(2);
			ISPD_Via.TopLayerRect.LB = make_pair(via->lefiVia::xl(2, 0), via->lefiVia::yl(2, 0));
			ISPD_Via.TopLayerRect.RT = make_pair(via->lefiVia::xh(2, 0), via->lefiVia::yh(2, 0));
		}
		else{
			printf("lefVia::Error With Via Layer Num is not 3\n");
			exit(1);
		}
		This_Design.Via_list.push_back(ISPD_Via);
	...
	void prtGeometry(lefiGeometries *geometry) {
		...
		for (i = 0; i < numItems; i++) {
			...
			case lefiGeomLayerE:
				TempPin.port.LAYER = geometry->lefiGeometries::getLayer(i);
				layer = geometry->lefiGeometries::getLayer(i);
				break;
			case lefiGeomRectE:{
				rect = geometry->lefiGeometries::getRect(i);
				if (rect->colorMask != 0) {
					printf("ERROR::Rect Mask\n");
					exit(1);
				}
				else {
					Rect rectangle;
					rectangle.LB = make_pair(rect->xl, rect->yl);
					rectangle.RT = make_pair(rect->xh, rect->yh);
					rectangle.Layer = layer;
					TempPin.port.rect_list.push_back(rectangle);
				}
				break;
			}
			}
	...
	int layerCB(lefrCallbackType_e c, lefiLayer* layer, lefiUserData) {
		...
		char temp[100];
		TempLayer.clear();
		//const char* name = layer->lefiLayer::name();
		strcpy(temp, layer->lefiLayer::name());
		//std::string name(layer->lefiLayer::name(), 0, 10);
		if (temp[0] == 'M'){
			TempLayer.Type = true;
			TempLayer.metalLayer.Name = layer->lefiLayer::name();
			if (layer->lefiLayer::hasDirection()){
				strcpy(temp, layer->lefiLayer::direction());
				if (temp[0] == 'H')
					TempLayer.metalLayer.DIRECTION = true;
				else TempLayer.metalLayer.DIRECTION = false;
			}
			if (layer->lefiLayer::hasArea())
				TempLayer.metalLayer.AREA = layer->lefiLayer::area();
			if (layer->lefiLayer::hasWidth())
				TempLayer.metalLayer.WIDTH = layer->lefiLayer::width();
			if (layer->lefiLayer::hasMinwidth())
				TempLayer.metalLayer.MINWIDTH = layer->lefiLayer::minwidth();
			if (layer->lefiLayer::hasPitch())
				TempLayer.metalLayer.PITCH.first = layer->lefiLayer::pitch();
			if (layer->lefiLayer::hasXYPitch())
				TempLayer.metalLayer.PITCH = make_pair(layer->lefiLayer::pitchX(), layer->lefiLayer::pitchY());
			if (layer->lefiLayer::hasSpacingNumber()) {
				for (i = 0; i < layer->lefiLayer::numSpacing(); i++) {
					if (layer->lefiLayer::hasSpacingEndOfLine(i)) {       // For version 5.7
						TempLayer.metalLayer.EOL.SPACING = layer->lefiLayer::spacing(i);
						TempLayer.metalLayer.EOL.ENDOFLINE = layer->lefiLayer::spacingEolWidth(i);
						TempLayer.metalLayer.EOL.WITHIN = layer->lefiLayer::spacingEolWithin(i);
					}
					else
						TempLayer.metalLayer.SPACING = layer->lefiLayer::spacing(i);
				}
			}
			if (layer->lefiLayer::numSpacingTable() > 0){
				spTable = layer->lefiLayer::spacingTable(0);
				if (spTable->lefiSpacingTable::isParallel()){
					parallel = spTable->lefiSpacingTable::parallel();  // Assume one PARALLELRUNLENGTH
					TempLayer.metalLayer.spacing_table.PARALLELRUNLENGTH = parallel->lefiParallel::length(0);
					for (j = 0; j < parallel->lefiParallel::numWidth(); j++)  // Assume one width one spacing
						TempLayer.metalLayer.spacing_table.WIDTH.push_back(
						make_pair(parallel->lefiParallel::width(j),
						parallel->lefiParallel::widthSpacing(j, 0)));
				}
			}
			This_Design.Layer_list.push_back(TempLayer);
			This_Design.Metal_Layer_Num++;
			TempLayer.clear();
		}
		else{
			TempLayer.Type = false;
			TempLayer.viaLayer.Name = layer->lefiLayer::name();
			TempLayer.viaLayer.TYPE = layer->lefiLayer::type();
			if (layer->lefiLayer::hasSpacingNumber()) {
				TempLayer.viaLayer.CUT_SPACING = layer->lefiLayer::spacing(0); // Assume one cut spacing
			}
			This_Design.Layer_list.push_back(TempLayer);
			This_Design.Via_Layer_Num++;
			TempLayer.clear();
		}
	...
	int macroClassTypeCB(lefrCallbackType_e c, const char* macroClassType,
		...
		checkType(c);
		// if ((long)ud != userData) dataError();
		TempMACRO.CLASS = macroClassType;
	...
	int macroCB(lefrCallbackType_e c, lefiMacro* macro, lefiUserData) {
		...
		if (macro->lefiMacro::hasSize())
			TempMACRO.SIZE_BY = make_pair(macro->lefiMacro::sizeX(), macro->lefiMacro::sizeY());
		if (macro->lefiMacro::hasOrigin())
			TempMACRO.ORIGIN = make_pair(macro->lefiMacro::originX(), macro->lefiMacro::originY());
	...
	int macroEndCB(lefrCallbackType_e c, const char* macroName, lefiUserData) {
		checkType(c);
		// if ((long)ud != userData) dataError();
		This_Design.Macro_list.push_back(TempMACRO);
		fprintf(fout, "END %s\n", macroName);
		return 0;
	}
	...
	int obstructionCB(lefrCallbackType_e c, lefiObstruction* obs,
		...
		for (int i = 0; i < numItems; i++){
			...
			case lefiGeomRectE:
				...
				else
				{
					...
					ISPD_rect.LB.first = rect->xl;
					ISPD_rect.LB.second = rect->yl;
					ISPD_rect.RT.first = rect->xh;
					ISPD_rect.RT.second = rect->yh;
					ISPD_rect.Layer = Layer;
					ISPD_Obs_Pin.port.rect_list.push_back(ISPD_rect);
				}
				break;
			default:
			{
		...
		TempMACRO.Pin_list.push_back(ISPD_Obs_Pin);
	...
	int pinCB(lefrCallbackType_e c, lefiPin* pin, lefiUserData) {
		...
		TempPin.clear();
		TempPin.Name = pin->lefiPin::name();
		if (pin->lefiPin::hasDirection())
			TempPin.DIRECTION = pin->lefiPin::direction();
		if (pin->lefiPin::hasUse())
			TempPin.USE = pin->lefiPin::use();
		if (pin->lefiPin::hasShape())
			TempPin.Shape = pin->lefiPin::shape();
		numPorts = pin->lefiPin::numPorts();
		for (i = 0; i < numPorts; i++) {
			geometry = pin->lefiPin::port(i);
			prtGeometry(geometry);
		}
		TempMACRO.Pin_list.push_back(TempPin);
...
int Lef_main(char * LefFilName, int argc , char **argv, Design &des) {
	printf("Strat Lef main\n");
	using namespace Lef;
	char* inFile[100];
	char* outFile;
	FILE* f;
