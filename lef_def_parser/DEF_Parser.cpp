...
namespace Def{
	...
	int compf(defrCallbackType_e c, defiComponent* co, defiUserData ud) {
		if (testDebugPrint) {
			...
			int i;
			// DEF_BEBE_COMPONENT
			Component ISPD_Component;
			checkType(c);
			if (ud != userData) dataError();
			//  missing GENERATE, FOREIGN
			fprintf(fout, "- %s %s ", co->id(),
				co->name());
			ISPD_Component.Name = co->name();
			ISPD_Component.ID = co->id();
			...
			if (co->isFixed()){
				...
				ISPD_Component.index = make_pair(co->placementX(), co->placementY());
				ISPD_Component.Dir = orientStr(co->placementOrient());
				def_design.Component_list.push_back(ISPD_Component);
			}
			if (co->isCover())
	...
	int netNamef(defrCallbackType_e c, const char* netName, defiUserData ud) {
		...
		Temp_Net_name.Net_name = netName;
		return 0;
	}
	...
	int netf(defrCallbackType_e c, defiNet* net, defiUserData ud) {
		...
		for (i = 0; i < net->numConnections(); i++) {
			...
			fprintf(fout, "( %s %s ) ", net->instance(i),
				net->pin(i));
			CPN.Component_Name = net->instance(i);
			CPN.Pin_Name = net->pin(i);
			Temp_Net_name.CPN_list.push_back(CPN);
		...
		def_design.Net_Name_list.push_back(Temp_Net_name);
		Temp_Net_name.clear();
	...
	int cls(defrCallbackType_e c, void* cl, defiUserData ud) {
		...
		case defrDieAreaCbkType:
			...
			fprintf(fout, "DIEAREA %d %d %d %d ;\n",
				...
				box->yh());
			// DEF_BEBE_DIEAREA
			def_design.Die_Area.LB = make_pair(box->xl(), box->yl());
			def_design.Die_Area.RT = make_pair(box->xh(), box->yh());
		...
		case defrPinCbkType:
			...
			if (testDebugPrint) {
				...
				fprintf(fout, "- %s + NET %s ", pin->pinName(),
					pin->netName());
				ISPD_Routing_Pin.Name = pin->pinName();
				ISPD_Routing_Pin.Net_belong = pin->netName();
				//         pin->changePinName("pinName");
				//         fprintf(fout, "%s ", pin->pinName());
				if (pin->hasDirection()){
					fprintf(fout, "+ DIRECTION %s ", pin->direction());
					ISPD_Routing_Pin.DIRECTION = pin->direction();
				}
				if (pin->hasUse()){
					fprintf(fout, "+ USE %s ", pin->use());
					ISPD_Routing_Pin.USE = pin->use();
				}
				if (pin->hasNetExpr())
				...
				if (pin->hasLayer()) {
					struct defiPoints points;
					if (pin->numLayer() > 1){
						printf("WARNING::PIN:: Pin in multi-layer\n");
						int pause; cin >> pause;
					}
					for (i = 0; i < pin->numLayer(); i++) {
						fprintf(fout, "\n  + LAYER %s ", pin->layer(i));
						ISPD_Routing_Pin.Layer = pin->layer(i);
						if (pin->layerMask(i))
							fprintf(fout, "MASK %d ",
							pin->layerMask(i));
						...
						ISPD_Routing_Pin.LayerBound.LB = make_pair(xl, yl);
						ISPD_Routing_Pin.LayerBound.RT = make_pair(xh, yh);
					}
				...
				if (pin->hasPlacement()) {
					if (pin->isPlaced()) {
						...
						ISPD_Routing_Pin.Place_Index = make_pair(pin->placementX(), pin->placementY());
						ISPD_Routing_Pin.Orient = orientStr(pin->orient());
						def_design.Routing_Pin_list.push_back(ISPD_Routing_Pin);
						ISPD_Routing_Pin.clear();
					}
					if (pin->isCover()) {
						fprintf(fout, "+ COVER ");
						fprintf(fout, "( %d %d ) %s ", pin->placementX(),
		...
		case defrRowCbkType:
			...
			ISPD_row.Name = row->name();
			ISPD_row.Macro = row->macro();
			ISPD_row.start_index = make_pair(row->x(), row->y());
			ISPD_row.Dir = orientStr(row->orient());
			if (row->hasDo()) {
				...
				ISPD_row.Do_By = make_pair(row->xNum(), row->yNum());
				ISPD_row.Step = make_pair(row->xStep(), row->yStep());
				def_design.Row_list.push_back(ISPD_row);
			}
			else
				fprintf(fout, ";\n");
			if (row->numProps() > 0) {
		...
		case defrTrackCbkType:
			...
			if (track->firstTrackMask()) {
				...
				fprintf(fout, "TRACKS %s %g DO %g STEP %g LAYER ",
					...
					track->xNum(), track->xStep());
				// DEF_BEBE_TRACKS
				ISPD_track.Dir = track->macro();
				ISPD_track.start_index = track->x();
				ISPD_track.Do = track->xNum();
				ISPD_track.Step = track->xStep();
			}
			ISPD_track.Layer = track->layer(0);
			def_design.Track_list.push_back(ISPD_track);
